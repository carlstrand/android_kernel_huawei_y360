...
static int g_duty = 0; //0-> torch ; 1-> falsh 
...
static int FL_Torch_Led_Enable(void)
{
    LEDS_DRV_DEBUG("FL_Torch_Led_Enable+ \n");
...
static int FL_Torch_Led_Disable(void)
{
	LEDS_DRV_DEBUG("FL_Torch_Led_Disable \n");
...
/***************************************************/
int flashlight_led_set_MT6333(char *led_name, int level)
{	
	if(level){
		FL_Torch_Led_Enable();
	}else{
		FL_Torch_Led_Disable();
	}
	return 0;
}
#endif
...
int flashlight_led_set_GPIO(char *led_name, int level)
{
