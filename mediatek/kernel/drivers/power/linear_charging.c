...
#define POST_CHARGING_TIME	 30 * 60 // 30mins
 ...
 kal_uint32 v_cc2topoff_threshold = V_CC2TOPOFF_THRES; 
   ...
   kal_uint32 v_recharge_threshold = RECHARGING_VOLTAGE; 
#endif
    ...
    {    
        ...
        else if (BMT_status.charger_type == STANDARD_CHARGER) 
        {
            ...
            g_temp_CC_value = AC_CHARGER_CURRENT;
				 ...
				 {
					g_temp_CC_value = AC_CHARGER_CURRENT_HIGH_TEMP_LIMIT; 
				 }
				 #endif
			}	
		#else
...
static void pchr_turn_on_charging (void)
{
    ...
    else
    {
        ...
        {    
			...
			#if !defined(MTK_JEITA_STANDARD_SUPPORT)           
				#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
					#if defined(WT_BAT_CHARGE_CV_TEMP_LIMIT)  
					if((BMT_status.temperature >= 0)&&(BMT_status.temperature <= 45))
					{
						cv_voltage = BATTERY_VOLT_04_362500_V;
					}
					else if((BMT_status.temperature > 45)&&(BMT_status.temperature <= 60))
					{
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
    ...
    BMT_status.total_charging_time += BAT_TASK_PERIOD;
		...
		{
			v_cc2topoff_threshold = V_CC2TOPOFF_THRES_HIGH_TEMP;
		}
		else
		{
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	if((BMT_status.temperature > 45)&&(BMT_status.temperature <= 60))
	{
		v_recharge_threshold = RECHARGING_VOLTAGE_HIGH_TEMP;
	}
	else
	{
		v_recharge_threshold = RECHARGING_VOLTAGE;
	}
   	if(BMT_status.bat_vol < v_recharge_threshold )   
