...
#ifdef WT_BQ2022A_BAT_ID_SUPPORT
		case POWER_SUPPLY_PROP_BatteryID:
...
static kal_bool mt_battery_0Percent_tracking_check_in_charging(void)
{
	kal_bool resetBatteryMeter = KAL_TRUE;
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[Battery] mt_battery_0Percent_tracking_check_in_charging(), VBAT < %d UI_SOC = (%d)\r\n", SYSTEM_OFF_VOLTAGE, BMT_status.UI_SOC);                
...
#ifdef WT_LOW_BAT_PERCENT_CHECKING   
extern kal_int32 fgauge_read_capacity_by_v(kal_int32 voltage); 
static kal_bool mt_vbat_soc_n_percent_tracking(void)
{
		...
		kal_uint32 vbat_checking_point_SOC = fgauge_read_capacity_by_v(vbat_checking_point);
			...
			if(BMT_status.UI_SOC <(vbat_checking_point_SOC+7) ) 
			{
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if( (BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC!=1)) && (BMT_status.charger_exist == KAL_FALSE)) 
#else
	...
	else
	{
		...
		else
		{
			...
			//	battery_xlog_printk(BAT_LOG_CRTI, "[Battery]mt_battery_Sync_UI_Percentage_to_Real ui_soc_sync_100_enable = KAL_TRUE \r\n ");
			}
		}
#endif
#else
...
#ifdef WT_0PERCENT_CHECK_IN_CHARGING     
				   if (BMT_status.bat_vol <= V_0PERCENT_TRACKING_in_charging) 
				   {
					   if(g_boot_mode == NORMAL_BOOT)
					   {
						   resetBatteryMeter = mt_battery_0Percent_tracking_check();
						   battery_xlog_printk(BAT_LOG_CRTI, "[Battery] mt_battery_0Percent_tracking_check_in_charging(), VBAT < %d BMT_status.bat_vol = (%d)\r\n", V_0PERCENT_TRACKING_in_charging, BMT_status.bat_vol);			   
					   }
				   } 
				   else 
				   {
...
#ifdef WT_HIGH_TEMP_CHARGING_SUPPORT  
				if(((BMT_status.temperature > 45)&&(BMT_status.temperature <= 60))&&(BMT_status.bat_full == KAL_TRUE))
				{
...
#ifdef WT_BQ2022A_BAT_ID_SUPPORT
PMU_STATUS mt_battery_CheckBatteryID(void)
{
    PMU_STATUS status = PMU_STATUS_OK;
    int bat_true = 0;
    int timeout = 50;
    ...
    {
        do {
            bat_true = bq2022a_read_bat_id();
        } while (bat_true && timeout--); 
        if((bat_true)&&(is_bat_id_byte == KAL_FALSE))
        {	// id NG
        	bat_id_ok=0;
        }
        else
    ...
    if(!bat_id_ok)
    {
    	battery_xlog_printk(BAT_LOG_CRTI,"billy , battery id check error !!!! \n\r"); 		
    	BMT_status.bat_charging_state = CHR_ERROR;
    	status = PMU_STATUS_FAIL; 		
    }
    return status;
...
#ifdef WT_HIGH_TEMP_CHARGING_SUPPORT
static void mt_battery_notify_high_temp_charging_full_warning_check(void)
{
   ...
   else
   {
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] mt_battery_notify_high_temp_charging_full_warning_check (%x)\n", g_BatteryNotifyCode);
...
#ifdef WT_HIGH_TEMP_CHARGING_SUPPORT
		mt_battery_notify_high_temp_charging_full_warning_check();
#endif
    }	
	else  /* for UI test */
	{
...
CHARGER_TYPE mt_charger_type_detection_again(void)
{
...
static void mt_battery_charger_detect_check(void)
{
	...
	if( upmu_is_chr_det() == KAL_TRUE )
    {
		...
		if(BMT_status.charger_type == CHARGER_UNKNOWN)
		{
			mt_charger_type_detection();
			#if 1
			if(((NONSTANDARD_CHARGER == BMT_status.charger_type)||(APPLE_2_1A_CHARGER == BMT_status.charger_type)||(APPLE_1_0A_CHARGER == BMT_status.charger_type)||(APPLE_0_5A_CHARGER == BMT_status.charger_type))&&(second_check == 0))
			{
				
			//	battery_xlog_printk(BAT_LOG_CRTI, "@hfs:mt_battery_charger_detect_check,222222\r\n");
				++second_check;
				
				mt_charger_type_detection_again();
				
		  //		battery_xlog_printk(BAT_LOG_CRTI, "[BAT_thread]@hfs scend mt_battery_charger_detect_check, CHR_Type_num=%d\r\n", BMT_status.charger_type);
    ...
    else 
    {
        ...
        mt_usb_disconnect();          
		#ifdef WT_UI_SOC_SYNC_FULL_IN_CHARGING //xuecheng for mandiandianyadi
...
void BAT_thread(void)
{
    ...
    #include <linux/hardware_info.h>
		if((bat_true == 0)||(is_bat_id_byte == KAL_TRUE)){
			bat_module_id = bq2022a_read_bat_module_id();
			switch(bat_module_id){
				case 1:
				...
				case 6:
					...
					break;
				case 7:
					sprintf(bat_id_buf,"%s","7 Feimaotui + Samsung(MI2A)");
					break;
				default:
					sprintf(bat_id_buf,"%s","0 Common battery");
					break;
			}			
		}
		else
		...
		battery_xlog_printk(BAT_LOG_CRTI,"bat_id_buf = %s",bat_id_buf);
		hardwareinfo_set_prop(HARDWARE_BAT, bat_id_buf);
#endif
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    /* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_StopCharging_Test);//stop charging
