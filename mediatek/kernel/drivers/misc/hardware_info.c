...
extern char hardware_info_lcdname[];
...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	case HARDWARE_LCD_GET:
		...
		hardwareinfo_set_prop(HARDWARE_LCD,hardware_info_lcdname);		
		break;
	...
	case HARDWARE_BT_GET:
		hardwareinfo_set_prop(HARDWARE_BT, "MT6627");
		hardwareinfo_num = HARDWARE_BT;
		break;
	case HARDWARE_WIFI_GET:
		hardwareinfo_set_prop(HARDWARE_WIFI, "MT6627");
