...
my $WT_CUSTOM_MEMORY_DEVICE_LIST_XLS="mediatek/config/${PROJECT}/MemoryDeviceList_${PLATFORM}.xls";

if (-e $WT_CUSTOM_MEMORY_DEVICE_LIST_XLS )
{
$MEMORY_DEVICE_LIST_XLS = $WT_CUSTOM_MEMORY_DEVICE_LIST_XLS ;
}
...
my $CUSTOM_EMI_C_LK = $CUSTOM_MEMORY_DEVICE_HDR;#wangfuqiang 
...
if ($os eq "windows")
{
	...
	$CUSTOM_EMI_C_LK = " mediatek/custom/common/lk/custom_emi.c";
...
#****************************************************************************
# generate custom_EMI.c in lk
#****************************************************************************
#if ($is_existed_c == 0)
{
    if ($is_existed_c == 1)
   {
		unlink ($CUSTOM_EMI_C_LK);
    }    
	my $temp_path = `dirname $CUSTOM_EMI_C_LK`;
    `mkdir -p $temp_path`;
    open (CUSTOM_EMI_C_LK, ">$CUSTOM_EMI_C_LK") or &error_handler("$CUSTOM_EMI_C_LK: file error!", __FILE__, __LINE__);
    print CUSTOM_EMI_C_LK &copyright_file_header();
    print CUSTOM_EMI_C_LK &description_file_header(                      "custom_emi.c",
          "This Module defines the EMI (external memory interface) related setting.",
                                                 "EMI auto generator". $EMIGEN_VERNO);
    print CUSTOM_EMI_C_LK &custom_EMI_c_file_body(\%CUSTOM_MEM_DEV_OPTIONS,\@MDL_INFO_LIST,\@MCP_LIST);
    close CUSTOM_EMI_C_LK or &error_handler("$CUSTOM_EMI_C_LK: file error!", __FILE__, __LINE__);
  print "\n$CUSTOM_EMI_C_LK is generated\n";
} 
...
sub DeviceListParser_LPSDRAM
{
    ...
    while ($iter<$num_part_found)
    {
        ...
        $EMMC_NAND_NAME[$id]                 = &xls_cell_value($Sheet, $_, $COLUMN_PART_NUMBER) ;#emmcnameinfo
...
# LPDDR3    
	...
	$nand_emmc_id_len = $ID_Length[$id];
        ...
        $EMMC_NAND_NAME[$id] = sprintf("\"%.30s\"",$EMMC_NAND_NAME[$id]); #emmcnameinfo
	      print "EMMC_NAND_NAME:$EMMC_NAND_NAME[$id]\n" ;#emmcnameinfo
        $EMI_SETTINGS[$id] = "\n\t//$PartNum\n\t{\n\t\t" ;
...
#3 LPDDR3    
		...
		$EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $EMMC_NAND_NAME[$id] . "\t\t/* EMMC_NAND_NAME */\n\t}" ;#emmcnameinfo
