...
###add for mhall, default no### 
WT_MHALL_SUPPORT = no

####add for version control####
WT_INNER_VERSION = SW_PROJECT_V001_M10_CUSTOMER_MP
WT_HARDWARE_VERSION = M10
WT_HARDWARE_VERSION_INNER = M10
WT_CUSTOMER_NAME = WINGTECH
WT_VERSION_NUM = v001
WT_PARENT_PROJECT_DIR = wingtech82_cwet_kk

WT_CMCC_VERSION = 4.2.010.P1.2013022
WT_CTA_VERSION = 4.2.010.P1.2013022.XM
WT_PRODUCTION_VERSION = 4.2.010.P1.2013022
WT_NETSWITCH_CODE = *#*#20120915#*#*
WT_CTA_VERSION_CODE = *#*#20120916#*#*
WT_CMCC_VERSION_CODE = *#*#20120917#*#*
WT_PRODUCT_VERSION_CODE = *#*#20120918#*#*
WT_PHONE_SN_SUPPORT = no
WT_OTA_USE_INNER_VERSION = yes
WT_GEMINI_DISP_SINGLE_IMEI = no
WT_FACTORY_RESET_CODE = *#*#13413#*#*

####add for factorymode control####
MTK_ENGMODE_TORCH_SUPPORT = yes
MTK_CAMERA_FLASHLIGHT = yes
MTK_HEADSET_HOOK_SUPPORT = yes
MTK_ENGMODE_LED_SUPPORT = yes
MTK_ENGMODE_GSENSORCALIB_SUPPORT = no
MTK_ENGMODE_G_SENSOR_SUPPORT = yes
MTK_ENGMODE_GYRO_SUPPORT = yes
MTK_ENGMODE_PROXIMITY_SUPPORT = yes
MTK_ENGMODE_OPTIC_SUPPORT = yes
MTK_ENGMODE_GPS_SUPPORT = yes
MTK_ENGMODE_MAGNETIC_SUPPORT = yes
MTK_HW_FACTORYMODE_SUPPORT = no

WT_VIBRATOR_ALWAYS_ON = yes
WT_PACKAGE_INSTALLER_ONCE = no
WT_FACTORYMODE_CODE = *#*#66#*#*
WT_STABILITY_CODE = *#*#99#*#*
WT_HW_LOCATION_SUPPORT = no
WT_RINGTONE_PICKER_SHOW_MORE_RINGTONES = yes
WT_MMS_SEND_EMPTY_MESSAGE = no
WT_CONTACT_NAME_NUMBER_MAXVALUE_FROM_SIM = no
WT_SMS_CENTER_NUMBER_EDIT_DISABLE_SUPPORT = no
WT_OVERSEA_PHONE_NUMBER_COMPARE_SUPPORT = no
WT_SPANISH_7BIT_LOOSY_SCHEME_SUPPORT = no
WT_EMAIL_CONFIG_FROM_XML = no
WT_MMS_CONFIG_FROM_XML = no
WT_MMS_RINGTONE_ATTACH_SLOT_ID = yes
WT_MMI_STKNAME_READFROMSIM = yes
WT_MMI_NOSIM_NEED_STKAPP = no
WT_SAVE_SIMRECORD_NEEDNAME_NUMBER = no
WT_HUAWEI_SHOW_MORE_AUDIOPROFILE_ICONS = yes
WT_HUAWEI_AUDIOPROFILES_DISABLE = no
WT_CELL_BROADCAST_WIDGET = no

# for huawei proj
WT_HUAWEI_PROJ = no
WT_CUSTOMER_HOTLINE_FUNC = no
WT_CHARGING_LIGHT_SUPPORT = yes

WT_HTC_WATERMARK = no
WT_CHOOSE_INSTALL_LOCATION = no

WT_SMS_SIGN_CONFIG_WITH_PRODUCT_MODEL = no
WT_TRANSPARENT_STATUS_BAR_SUPPORT = no
WT_HUAWEI_AUDIO_DOUBLE_CHANNELS = no
WT_ILLEGAL_RINGTONES_PICKER = no
WT_HUAWEI_SILENT_MODE_TOGGLE_ACTION = no
WT_CUSTOM_SIGNAL_LEVEL_SUPPORT = yes
WT_CONTACT_CHECK_NAME_PHONE = no
WT_SHOW_SETTINGS_PANEL_NO_NOTIFICATIONS = yes
WT_TEMPRATURE_POWERING_OFF_SUPPORT = yes
WT_BROWSER_BOOKMARKS_BY_XML_MCCMNC = no
WT_STK_SELECTION = no
WT_SMS_VIETNAM_SUPPORT = no

#yubosong added for huawei Europe earphone security
WT_EU_EARPHONE_SECURITY = no

WT_BRIGHTNESS_MAPPING_WITH_LCM = yes
WT_BACKLIGHT_DISABLE_WITH_NOLCM = yes
...
#add by zhihui.wang for WT_LAUNCHER2_ICON_WITH_BG and WT_LAUNCHER2_ICON_WITH_CUSTOMER_ICON
WT_LAUNCHER2_ICON_WITH_BG = no
WT_LAUNCHER2_ICON_WITH_CUSTOMER_ICON = no

###add for huawei nff test##
WT_HWNFF_SUPPORT = no
MTK_ROOT_MONITOR_SUPPORT = no
WT_AUTO_REPAIR    = no
WT_HUAWEI_DROP_PHONE_SUPPORT = no
WT_HUAWEI_GLOBAL = no
MTK_MEMORY_COMPRESSION_SUPPORT = no
MTK_TINY_UTIL = no
WT_HUAWEI_S98360A1 = no
WT_HUAWEI_S98360C1 = no
WT_HUAWEI_S98360B1 = no
WT_HUAWEI_S98360_U03A1 = no
WT_HUAWEI_S98360_U23B1 = no
WT_HUAWEI_S98360_U23B2 = no
WT_HUAWEI_S98360_U23B3 = no
WT_CTP_GESTURE_SUPPORT=no
WT_USB_STORAGE_ADD_USB_DEBUG = no
WT_OTA_SUPPORT = no
WT_HUAWEI_LOW_STORAGE_SCHEME = no
WT_HUAWEI_INTERNAL_SD_CARD_NO_SPACE_REMIND = no
WT_HUAWEI_SD_CARD_NO_SPACE_REMIND = no
WT_ANIMATION_ROLL = yes
WT_ANIMATION_ROLL_PERCENT_NUMBER = no

WT_HUAWEI_CHANGE_BG = yes
WT_HUAWEI_IP_OVERSEA = yes
#add by huangyabin for huawei audio regional customization
WT_AUDIO_REGIONAL_CUSTOMIZATION = no
MTK_LAUNCHER = Launcher2

WT_BUILD_NUMBER = WINGTECH
WT_PRODUCT_BRAND = WINGTECH
WT_PRODUCT_NAME = WINGTECH
WT_PRODUCT_DEVICE = WINGTECH
WT_BUILD_ID = WINGTECH
PRODUCT_MODEL = WINGTECH
PRODUCT_MANUFACTURER = WINGTECH

WT_HUAWEI_SDN_MENU = no
WT_SPEEDDIAL_FIX3 = no

#add by fengshiheng for huawei DTV feature
WT_ENABLE_NMI_DTV = no
WT_USER_NAME = root
WT_HOST_NAME = ubuntu
WT_REMOVE_PREFERRED_NETWORKS_MENU = no
