...
#include "ov5670mipirawyiweili_Sensor.h"
#include "ov5670mipirawyiweili_Camera_Sensor_para.h"
#include "ov5670mipirawyiweili_CameraCustomized.h"
...
kal_uint32 OV5670_FeatureControl_PERIOD_PixelNum_YIWEILI=OV5670_PV_PERIOD_PIXEL_NUMS;
kal_uint32 OV5670_FeatureControl_PERIOD_LineNum_YIWEILI=OV5670_PV_PERIOD_LINE_NUMS;

UINT16 OV5670_VIDEO_MODE_TARGET_FPS_YIWEILI = 30;

MSDK_SCENARIO_ID_ENUM OV5670CurrentScenarioId_YIWEILI = MSDK_SCENARIO_ID_CAMERA_PREVIEW;
MSDK_SENSOR_CONFIG_STRUCT OV5670SensorConfigData_YIWEILI;
static OV5670_PARA_STRUCT OV5670;
kal_uint32 OV5670_FAC_SENSOR_REG_YIWEILI;


SENSOR_REG_STRUCT OV5670SensorCCT_YIWEILI[]=CAMERA_SENSOR_CCT_DEFAULT_VALUE;
SENSOR_REG_STRUCT OV5670SensorReg_YIWEILI[ENGINEER_END]=CAMERA_SENSOR_REG_DEFAULT_VALUE;

#define OV5670_OTP
#define OV5670_TEST_PATTERN_CHECKSUM 0x8c753bb4 //0x5ec94641//0xca3667da //0x5d8082f0 //0x75bef806 //0xa2230d9f    //0xf5e2f1ce
kal_bool OV5670_During_testpattern_YIWEILI = KAL_FALSE;
...
kal_uint16 OV5670_read_cmos_sensor_YIWEILI(kal_uint32 addr)
{
...
int OV5670MIPI_check_awb_gain_YIWEILI(){
	...
	R_Gain_H=(UINT8)OV5670_read_cmos_sensor_YIWEILI(0x5032);
	R_Gain_L=(UINT8)OV5670_read_cmos_sensor_YIWEILI(0x5033);
	G_Gain_H=(UINT8)OV5670_read_cmos_sensor_YIWEILI(0x5034);
	G_Gain_L=(UINT8)OV5670_read_cmos_sensor_YIWEILI(0x5035);
	B_Gain_H=(UINT8)OV5670_read_cmos_sensor_YIWEILI(0x5036);
	B_Gain_L=(UINT8)OV5670_read_cmos_sensor_YIWEILI(0x5037);

	OV5670DB("qunhui test R_Gain_H=0x%02x R_Gain_L=0x%02x G_Gain_H=0x%02x G_Gain_L=0x%02x B_Gain_H=0x%02x B_Gain_L=0x%02x\r\n",
					R_Gain_H,R_Gain_L,G_Gain_H,G_Gain_L,B_Gain_H,B_Gain_L);
}
void OV5670_OTP_read_begin_YIWEILI(void)
{
	// set reg 0x5002[3] to '0'
	int temp1;
	temp1 = OV5670_read_cmos_sensor_YIWEILI(0x5002);
	OV5670_write_cmos_sensor(0x5002, (temp1 &(~0x08)));
	mdelay(5);
}

void OV5670_OTP_read_end_YIWEILI(void)
{
	// set reg 0x5002[3] to '1'
	int temp1;
	temp1 = OV5670_read_cmos_sensor_YIWEILI(0x5002);
	OV5670_write_cmos_sensor(0x5002, (0x08 | temp1));
}
...
int check_otp_info_YIWEILI(int index)
{
	int flag, i;
	int address_start = 0x7040;
	int address_end = 0x7040;
	OV5670_OTP_read_begin_YIWEILI();
	// read otp into buffer
	OV5670_write_cmos_sensor(0x3d84, 0xc0); // program disable, manual mode

	 address_start = 0x7040 - (index-1)*0x10;
	 address_end = 0x7040 - (index-1)*0x10;
	//partial mode OTP write start address
	OV5670_write_cmos_sensor(0x3d88, (address_start>>8));
	OV5670_write_cmos_sensor(0x3d89, (address_start & 0xff));
	// partial mode OTP write end address
	OV5670_write_cmos_sensor(0x3d8A, (address_end>>8));

	OV5670_write_cmos_sensor(0x3d8B, (address_end & 0xff));
	OV5670_write_cmos_sensor(0x3d81, 0x01); // read otp
	mdelay(5);
	flag = OV5670_read_cmos_sensor_YIWEILI(address_start);
	// clear otp buffer
	for (i=address_start;i<=address_end;i++) {
		OV5670_write_cmos_sensor(i, 0x00);
	}
	OV5670_OTP_read_end_YIWEILI();
	OV5670DB("qunhui check_otp_info_YIWEILI index=%d  flag = %d \n",index,flag);
	if (flag == 0x55) {
		return 2;
	}
	else {
...
int check_otp_wb_YIWEILI(int index)
{
	int flag, i;
	...
	//select group
	flag = OV5670_read_cmos_sensor_YIWEILI(0x7020);
	OV5670DB("AWB otp flag = %d \n",flag);
...
// return: 0,
int read_otp_info_YIWEILI(int index, struct otp_struct *otp_ptr)
{
	int i;
	int address_start;
	int address_end;
	...
	mdelay(5);
	(*otp_ptr).module_integrator_id =OV5670_read_cmos_sensor_YIWEILI(address_start+1) & 0x0f;
	(*otp_ptr).lens_id = (OV5670_read_cmos_sensor_YIWEILI(address_start + 1)>>4) & 0x03;
	(*otp_ptr).production_year = OV5670_read_cmos_sensor_YIWEILI(address_start + 11);
	(*otp_ptr).production_month = OV5670_read_cmos_sensor_YIWEILI(address_start + 12);
	(*otp_ptr).production_day = OV5670_read_cmos_sensor_YIWEILI(address_start + 13);
	// clear otp buffer
	for (i=address_start;i<=address_end;i++) {
		OV5670_write_cmos_sensor(i, 0x00);
	}
...
// otp_ptr: pointer of otp_struct
// return: 0,
int read_otp_wb_YIWEILI(UINT32 index, struct otp_struct *otp_ptr)
{
	UINT32 i, temp;
	UINT32 address_start;
	UINT32 address_end;
	//select group
	 address_start = 0x7040 - (index-1)*0x10 + 2;
	 address_end = address_start + 6;
	OV5670_OTP_read_begin_YIWEILI();
	// read otp into buffer
	OV5670_write_cmos_sensor(0x3d84, 0xc0); // program disable, manual mode
	//partial mode OTP write start address
	...
	OV5670_write_cmos_sensor(0x3d81, 0x01); // load otp into buffer
	mdelay(5);
	temp = OV5670_read_cmos_sensor_YIWEILI(address_start+6);
	otp_ptr->rg_ratio = OV5670_read_cmos_sensor_YIWEILI(address_start) + ((temp & 0x03) << 8);
	temp = OV5670_read_cmos_sensor_YIWEILI(address_start+6);
	otp_ptr->bg_ratio = OV5670_read_cmos_sensor_YIWEILI(address_start + 1) + (((temp>>2) & 0x03)<<8);
	//temp = OV5670_read_cmos_sensor_YIWEILI(address_start+21);
	//otp_ptr->r_golden = (OV5670_read_cmos_sensor_YIWEILI(address_start + 20)<<2) + ((temp>>6) & 0x03);
	//temp = OV5670_read_cmos_sensor_YIWEILI(address_start+23);
	//otp_ptr->b_golden = (OV5670_read_cmos_sensor_YIWEILI(address_start + 22)<<2) + ((temp>>6) & 0x03);
	temp = OV5670_read_cmos_sensor_YIWEILI(address_start+6);
	otp_ptr->gr_golden = OV5670_read_cmos_sensor_YIWEILI(address_start + 2) + (((temp>>4) & 0x03)<<8);
	temp = OV5670_read_cmos_sensor_YIWEILI(address_start+6);
	otp_ptr->gb_golden = (OV5670_read_cmos_sensor_YIWEILI(address_start + 3)<<2) + (((temp>>6) & 0x03)<<8);
	// clear otp buffer
	...
	//OV5670DB("AWB otp: r_golden = %d \n",otp_ptr->r_golden);
	//OV5670DB("AWB otp: b_golden = %d \n",otp_ptr->b_golden);
	OV5670DB("qunhui AWB otp: gr_golden = %d \n",otp_ptr->gr_golden);
	OV5670DB("qunhui AWB otp: gb_golden = %d \n",otp_ptr->gb_golden);
	OV5670DB("qunhui AWB otp: rg_ratio = %d \n",otp_ptr->rg_ratio);
	OV5670DB("qunhui AWB otp: bg_ratio = %d \n",otp_ptr->bg_ratio);
...
// return 0;
int update_awb_gain_YIWEILI(UINT32 R_gain, UINT32 G_gain, UINT32 B_gain)
{
	...
	OV5670DB("qunhui update_awb_gain_YIWEILI R_Gain_H=0x%04x;R_Gain_L= 0x%04x;G_Gain_H=0x%04x;G_Gain_L=0x%04x;B_Gain_H=0x%04x;B_Gain_L=0x%04x\n",
		R_Gain_H,R_Gain_L,G_Gain_H,G_Gain_L,B_Gain_H,B_Gain_L);
	if (R_gain>=0x400) {
		OV5670_write_cmos_sensor(0x5032, R_Gain_H);
		OV5670_write_cmos_sensor(0x5033, R_Gain_L);
	}
	if (G_gain>=0x400) {
		OV5670_write_cmos_sensor(0x5034, G_Gain_H);
		OV5670_write_cmos_sensor(0x5035, G_Gain_L);
	}
	if (B_gain>=0x400) {
		OV5670_write_cmos_sensor(0x5036, B_Gain_H);
		OV5670_write_cmos_sensor(0x5037, B_Gain_L);
	}
	return 0;
}
...
// check first OTP with valid data
	printk("qunhui test OTP index----%d---\r\n",otp_index);
	if (otp_index>4) {
	// no valid wb OTP data
		printk("qunhui test no wb OTP-------\r\n");
		return 1;
	}
	read_otp_wb_YIWEILI(otp_index, &current_otp);
	...
	//BG_Ratio_Typical=0x128;//0x120;
	//RG_Ratio_Typical=0x133;//0x12c;
	RG_Ratio_Typical = current_otp.gr_golden;
	BG_Ratio_Typical = current_otp.gb_golden;
	
	OV5670DB("qunhui test ration rg bg [0x%x,0x%x]\r\n",rg,bg);
	//OV5670DB("qunhui test r b gr gb[0x%x,0x%x,0x%x,0x%x]\r\n",r_golden,b_golden,gr_golden,gb_golden);
	//OV5670DB("qunhui test BG_Ratio_Typical=0x%04x  RG_Ratio_Typical=0x%04x\r\n",BG_Ratio_Typical,RG_Ratio_Typical);
	//calculate G gain
	//0x400 = 1x gain
	...
	OV5670MIPI_check_awb_gain_YIWEILI();
	OV5670DB("qunhui test uR_gain=0x%04x,uG_gain=0x%04x,uB_gain=0x%04x\r\n",R_gain,G_gain,B_gain);
	update_awb_gain_YIWEILI(R_gain, G_gain, B_gain);
	OV5670MIPI_check_awb_gain_YIWEILI();
	return 0;
}
...
void OV5670_Init_Para_YIWEILI(void)
{
...
kal_uint32 GetOV5670LineLength_YIWEILI(void)
{
...
kal_uint32 GetOV5670FrameLength_YIWEILI(void)
{
...
kal_uint32 OV5670_CalcExtra_For_ShutterMargin_YIWEILI(kal_uint32 shutter_value,kal_uint32 shutterLimitation)
{
...
kal_uint32 OV5670_CalcFrameLength_For_AutoFlicker_YIWEILI(void)
{

    kal_uint32 AutoFlicker_min_framelength = 0;

	switch(OV5670CurrentScenarioId_YIWEILI)
	{
		...
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			if(OV5670_VIDEO_MODE_TARGET_FPS_YIWEILI==30)
			{
			...
			else if(OV5670_VIDEO_MODE_TARGET_FPS_YIWEILI==15)
			{
	...
	OV5670DB("AutoFlicker_min_framelength =%d,OV5670CurrentScenarioId_YIWEILI =%d\n", AutoFlicker_min_framelength,OV5670CurrentScenarioId_YIWEILI);
...
void OV5670_write_shutter_YIWEILI(kal_uint32 shutter)
{
    ...
    line_length  = GetOV5670LineLength_YIWEILI();
	frame_length = GetOV5670FrameLength_YIWEILI();
	
	max_shutter  = frame_length-OV5670_SHUTTER_MARGIN;

    frame_length = frame_length + OV5670_CalcExtra_For_ShutterMargin_YIWEILI(shutter,max_shutter);
	


	if(OV5670.OV5670AutoFlickerMode == KAL_TRUE)
	{
        min_framelength = OV5670_CalcFrameLength_For_AutoFlicker_YIWEILI();
	...
	OV5670_FeatureControl_PERIOD_PixelNum_YIWEILI = line_length;
	OV5670_FeatureControl_PERIOD_LineNum_YIWEILI = frame_length;
...
static kal_uint16 OV5670Reg2Gain_YIWEILI(const kal_uint16 iReg)
{
    kal_uint16 iGain =0; 
...
static kal_uint16 OV5670Gain2Reg_YIWEILI(const kal_uint16 Gain)
{
...
void write_OV5670_gain_YIWEILI(kal_uint16 gain)
{
	kal_uint16 iGain =1;
	...
	//read_gain=(((OV5670_read_cmos_sensor_YIWEILI(0x3508)&0x1F) << 8) | OV5670_read_cmos_sensor_YIWEILI(0x3509));
	//OV5670DB("[OV5670_SetGain_YIWEILI]0x3508|0x3509=0x%x \n",read_gain);
	//OV5670DB("[OV5670_SetGain_YIWEILI]0x366a=%d \n",(OV5670_read_cmos_sensor_YIWEILI(0x366a)));

	return;

}

void OV5670_SetGain_YIWEILI(UINT16 iGain)
{
	unsigned long flags;
	spin_lock_irqsave(&ov5670mipiraw_drv_lock,flags);

	OV5670DB("OV5670_SetGain_YIWEILI iGain = %d :\n ",iGain);
	
	OV5670.realGain = iGain;
	OV5670.sensorGlobalGain = OV5670Gain2Reg_YIWEILI(iGain);
	spin_unlock_irqrestore(&ov5670mipiraw_drv_lock,flags);

	write_OV5670_gain_YIWEILI(OV5670.sensorGlobalGain);
	OV5670DB(" [OV5670_SetGain_YIWEILI]OV5670.sensorGlobalGain=0x%x,OV5670.realGain =%d",OV5670.sensorGlobalGain,
		OV5670.realGain); 

	//temperature test
	//OV5670_write_cmos_sensor(0x4d12,0x01);
	//OV5670DB("Temperature read_reg  0x4d13  =%x \n",OV5670_read_cmos_sensor_YIWEILI(0x4d13));
}   

kal_uint16 read_OV5670_gain_YIWEILI(void)
{
	...
	OV5670.realGain = OV5670Reg2Gain_YIWEILI(OV5670.sensorGlobalGain);
	spin_unlock(&ov5670mipiraw_drv_lock);
...
#if 1
void OV5670_camera_para_to_sensor_YIWEILI(void)
{
    kal_uint32    i;
    for(i=0; 0xFFFFFFFF!=OV5670SensorReg_YIWEILI[i].Addr; i++)
    {
        OV5670_write_cmos_sensor(OV5670SensorReg_YIWEILI[i].Addr, OV5670SensorReg_YIWEILI[i].Para);
    }
    for(i=ENGINEER_START_ADDR; 0xFFFFFFFF!=OV5670SensorReg_YIWEILI[i].Addr; i++)
    {
        OV5670_write_cmos_sensor(OV5670SensorReg_YIWEILI[i].Addr, OV5670SensorReg_YIWEILI[i].Para);
    }
    for(i=FACTORY_START_ADDR; i<FACTORY_END_ADDR; i++)
    {
        OV5670_write_cmos_sensor(OV5670SensorCCT_YIWEILI[i].Addr, OV5670SensorCCT_YIWEILI[i].Para);
    }
}

void OV5670_sensor_to_camera_para_YIWEILI(void)
{
    kal_uint32    i, temp_data;
    for(i=0; 0xFFFFFFFF!=OV5670SensorReg_YIWEILI[i].Addr; i++)
    {
         temp_data = OV5670_read_cmos_sensor_YIWEILI(OV5670SensorReg_YIWEILI[i].Addr);
		 spin_lock(&ov5670mipiraw_drv_lock);
		 OV5670SensorReg_YIWEILI[i].Para =temp_data;
		 spin_unlock(&ov5670mipiraw_drv_lock);
    }
    for(i=ENGINEER_START_ADDR; 0xFFFFFFFF!=OV5670SensorReg_YIWEILI[i].Addr; i++)
    {
...
kal_int32  OV5670_get_sensor_group_count_YIWEILI(void)
{
    return GROUP_TOTAL_NUMS;
}

void OV5670_get_sensor_group_info_YIWEILI(kal_uint16 group_idx, kal_int8* group_name_ptr, kal_int32* item_count_ptr)
{
   switch (group_idx)
   {
        case PRE_GAIN:
...
void OV5670_get_sensor_item_info_YIWEILI(kal_uint16 group_idx,kal_uint16 item_idx, MSDK_SENSOR_ITEM_INFO_STRUCT* info_ptr)
{
    ...
    switch (group_idx)
    {
        case PRE_GAIN:
           switch (item_idx)
          {
            ...
            temp_para= OV5670SensorCCT_YIWEILI[temp_addr].Para;
...
kal_bool OV5670_set_sensor_item_info_YIWEILI(kal_uint16 group_idx, kal_uint16 item_idx, kal_int32 ItemValue)
{
   kal_uint16  temp_gain=0,temp_addr=0, temp_para=0;
...
//             temp_para=(temp_gain * OV5670.sensorBaseGain + BASEGAIN/2)/BASEGAIN;
		  ...
		  spin_lock(&ov5670mipiraw_drv_lock);
          OV5670SensorCCT_YIWEILI[temp_addr].Para = temp_para;
		  spin_unlock(&ov5670mipiraw_drv_lock);
          OV5670_write_cmos_sensor(OV5670SensorCCT_YIWEILI[temp_addr].Addr,temp_para);
        ...
        case REGISTER_EDITOR:
            switch (item_idx)
            {
                case 0:
					spin_lock(&ov5670mipiraw_drv_lock);
                    OV5670_FAC_SENSOR_REG_YIWEILI=ItemValue;
					spin_unlock(&ov5670mipiraw_drv_lock);
                    break;
                case 1:
                    OV5670_write_cmos_sensor(OV5670_FAC_SENSOR_REG_YIWEILI,ItemValue);
                    break;
                default:
                    ASSERT(0);
            }
...
static void OV5670_SetDummy_YIWEILI( const kal_uint32 iPixels, const kal_uint32 iLines )
{
	kal_uint32 line_length = 0;
	kal_uint32 frame_length = 0;
...
void OV5670Preview_YIWEILISetting(void)
{
	OV5670DB(" OV5670Preview_YIWEILISetting_2lane enter\n");
...
void OV5670Video_YIWEILISetting(void)
{
	OV5670DB(" OV5670videoSetting_2lane enter:video/preview sync\n");

	OV5670Preview_YIWEILISetting();
}



void OV5670Capture_YIWEILISetting(void)
{
	OV5670DB("OV5670Capture_YIWEILISetting_2lane enter\n");
...
static void OV5670_Sensor_Init_YIWEILI(void)
{
	OV5670DB("OV5670_Sensor_Init_YIWEILI_2lane enter\n");
	...
	OV5670_write_cmos_sensor(0x0100, 0x01); //open stream to read OTP

}


UINT32 OV5670Open_YIWEILI(void)
{

	volatile signed int i;
	kal_uint16 sensor_id = 0;

	OV5670DB("qunhui OV5670 Open enter :\n ");
	OV5670_write_cmos_sensor(0x0103,0x01);// Reset sensor
    mdelay(2);

	for(i=0;i<2;i++)
	{
		sensor_id = (OV5670_read_cmos_sensor_YIWEILI(0x300B)<<8)|OV5670_read_cmos_sensor_YIWEILI(0x300C) + 1;
		OV5670DB("qunhui OV5670 READ ID :%x",sensor_id);
		if(sensor_id != OV5670_SENSOR_YIWEILI_ID)
		{
	...
	OV5670_Sensor_Init_YIWEILI();
    OV5670_Init_Para_YIWEILI();
#ifdef OV5670_OTP
	update_otp_wb_YIWEILI();
#endif
	OV5670DB("qunhui OV5670Open_YIWEILI exit :\n ");

    return ERROR_NONE;
}


UINT32 OV5670GetSensorID_YIWEILI(UINT32 *sensorID)
{
    int  retry = 2;
    int  i,temp=0;
	struct otp_struct current_otp;
	
	OV5670DB("qunhui OV5670GetSensorID_YIWEILI enter :\n ");
    mdelay(5);

    do {
        *sensorID = (OV5670_read_cmos_sensor_YIWEILI(0x300B)<<8)|OV5670_read_cmos_sensor_YIWEILI(0x300C) + 1;
        if (*sensorID == (OV5670_SENSOR_YIWEILI_ID) )
        	{
        		OV5670DB("qunhui Sensor ID = 0x%04x\n", *sensorID);
            	break;
        	}
        OV5670DB("qunhui Read Sensor ID Fail = 0x%04x\n", *sensorID);
        retry--;
    } while (retry > 0);
	...
	*/
	 *sensorID = 0xFFFFFFFF;
     return ERROR_SENSOR_CONNECT_FAIL;
		...
		read_otp_info_YIWEILI(otp_index, &current_otp);
        OV5670DB("Read qunhui Module ID = 0x%x, otp_index=%d\n", current_otp.module_integrator_id,otp_index);
		if(0x0a != current_otp.module_integrator_id){
			 *sensorID = 0xFFFFFFFF;
		     return ERROR_SENSOR_CONNECT_FAIL;
		}
...
void OV5670_SetShutter_YIWEILI(kal_uint32 iShutter)
{
   ...
   OV5670_write_shutter_YIWEILI(iShutter);
   return;
}



UINT32 OV5670_read_shutter_YIWEILI(void)
{

	kal_uint16 temp_reg1, temp_reg2 ,temp_reg3;
	UINT32 shutter =0;
	temp_reg1 = OV5670_read_cmos_sensor_YIWEILI(0x3500);    // AEC[b19~b16]
	temp_reg2 = OV5670_read_cmos_sensor_YIWEILI(0x3501);    // AEC[b15~b8]
	temp_reg3 = OV5670_read_cmos_sensor_YIWEILI(0x3502);    // AEC[b7~b0]
...
void OV5670_NightMode_YIWEILI(kal_bool bEnable)
{

}

UINT32 OV5670Close_YIWEILI(void)
{
...
void OV5670SetFlipMirror(kal_int32 imgMirror)
{
	...
	mirror= OV5670_read_cmos_sensor_YIWEILI(0x3820);
	flip  = OV5670_read_cmos_sensor_YIWEILI(0x3821);
...
UINT32 OV5670Preview_YIWEILI(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{

	OV5670DB("OV5670Preview_YIWEILI enter:");
	...
	OV5670.DummyPixels = 0;
	OV5670.DummyLines = 0 ;
	OV5670_FeatureControl_PERIOD_PixelNum_YIWEILI=OV5670_PV_PERIOD_PIXEL_NUMS+ OV5670.DummyPixels;
	OV5670_FeatureControl_PERIOD_LineNum_YIWEILI=OV5670_PV_PERIOD_LINE_NUMS+OV5670.DummyLines;
	...
	OV5670DB("OV5670Preview_YIWEILI exit:\n");
...
UINT32 OV5670Video_YIWEILI(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{

	OV5670DB("OV5670Video_YIWEILI enter:");

	OV5670Video_YIWEILISetting();

	spin_lock(&ov5670mipiraw_drv_lock);
	OV5670.sensorMode = SENSOR_MODE_VIDEO;
	OV5670_FeatureControl_PERIOD_PixelNum_YIWEILI=OV5670_VIDEO_PERIOD_PIXEL_NUMS+ OV5670.DummyPixels;
	OV5670_FeatureControl_PERIOD_LineNum_YIWEILI=OV5670_VIDEO_PERIOD_LINE_NUMS+OV5670.DummyLines;
	OV5670.imgMirror = sensor_config_data->SensorImageMirror;
	spin_unlock(&ov5670mipiraw_drv_lock);
    ...
    mdelay(40);//THIS DELAY SHOULD BE NEED BY CTS OR MONKEY
	OV5670DB("OV5670Video_YIWEILI exit:\n");
    return ERROR_NONE;
}


UINT32 OV5670Capture_YIWEILI(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	{
		OV5670DB("OV5670Capture_YIWEILI BusrtShot / ZSD!!!\n");
	}
	else
	{
		OV5670DB("OV5670Capture_YIWEILI enter:\n");

		OV5670Capture_YIWEILISetting();
	    mdelay(40);//THIS DELAY SHOULD BE NEED BY CTS OR MONKEY
		...
		OV5670.DummyPixels = 0;
		OV5670.DummyLines = 0 ;
		OV5670_FeatureControl_PERIOD_PixelNum_YIWEILI = OV5670_FULL_PERIOD_PIXEL_NUMS + OV5670.DummyPixels;
		OV5670_FeatureControl_PERIOD_LineNum_YIWEILI = OV5670_FULL_PERIOD_LINE_NUMS + OV5670.DummyLines;
		spin_unlock(&ov5670mipiraw_drv_lock);

		//OV5670SetFlipMirror(sensor_config_data->SensorImageMirror);

		OV5670DB("OV5670Capture_YIWEILI exit:\n");
	}

	if(OV5670_During_testpattern_YIWEILI == KAL_TRUE)
	{
...
UINT32 OV5670GetResolution_YIWEILI(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{

    OV5670DB("OV5670GetResolution_YIWEILI!!\n");
...
UINT32 OV5670GetInfo_YIWEILI(MSDK_SCENARIO_ID_ENUM ScenarioId,
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    memcpy(pSensorConfigData, &OV5670SensorConfigData_YIWEILI, sizeof(MSDK_SENSOR_CONFIG_STRUCT));

    return ERROR_NONE;
}   /* OV5670GetInfo_YIWEILI() */



UINT32 OV5670Control_YIWEILI(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
		spin_lock(&ov5670mipiraw_drv_lock);
		OV5670CurrentScenarioId_YIWEILI = ScenarioId;
		spin_unlock(&ov5670mipiraw_drv_lock);
		
		OV5670DB("OV5670CurrentScenarioId_YIWEILI=%d\n",OV5670CurrentScenarioId_YIWEILI);

	switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            OV5670Preview_YIWEILI(pImageWindow, pSensorConfigData);
            break;
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			OV5670DB("OV5670 video_preiew sync\n");
			OV5670Video_YIWEILI(pImageWindow, pSensorConfigData);
			break;
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
            OV5670Capture_YIWEILI(pImageWindow, pSensorConfigData);
            break;
    ...
    return ERROR_NONE;
} /* OV5670Control_YIWEILI() */



kal_uint32 OV5670_SET_FrameLength_ByVideoMode_YIWEILI(UINT16 Video_TargetFps)
{
    UINT32 frameRate = 0;
...
UINT32 OV5670SetVideoMode_YIWEILI(UINT16 u2FrameRate)
{

    kal_uint32 MIN_Frame_length =0,frameRate=0,extralines=0;
    OV5670DB("[OV5670SetVideoMode_YIWEILI] frame rate = %d\n", u2FrameRate);

	spin_lock(&ov5670mipiraw_drv_lock);
	OV5670_VIDEO_MODE_TARGET_FPS_YIWEILI=u2FrameRate;
	spin_unlock(&ov5670mipiraw_drv_lock);
    ...
    if(OV5670.sensorMode == SENSOR_MODE_VIDEO)//video ScenarioId recording
    {
		...
		OV5670_SetDummy_YIWEILI(OV5670.DummyPixels,extralines);
    }
	
	OV5670DB("[OV5670SetVideoMode_YIWEILI]MIN_Frame_length=%d,OV5670.DummyLines=%d\n",MIN_Frame_length,OV5670.DummyLines);

    return KAL_TRUE;
}


UINT32 OV5670SetAutoFlickerMode_YIWEILI(kal_bool bEnable, UINT16 u2FrameRate)
{
...
UINT32 OV5670SetTestPatternMode_YIWEILI(kal_bool bEnable)
{
    OV5670DB("[OV5670SetTestPatternMode_YIWEILI] Test pattern enable:%d\n", bEnable);
    if(bEnable == KAL_TRUE)
    {
        OV5670_During_testpattern_YIWEILI = KAL_TRUE;
	...
	{
        OV5670_During_testpattern_YIWEILI = KAL_FALSE;
...
* 
*************************************************************************/
UINT32 OV5670MIPISetMaxFramerateByScenario_YIWEILI(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	kal_uint32 pclk;
	kal_int16 dummyLine;
	kal_uint16 lineLength,frameHeight;
		
	OV5670DB("OV5670MIPISetMaxFramerateByScenario_YIWEILI: scenarioId = %d, frame rate = %d\n",scenarioId,frameRate);
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			OV5670.sensorMode = SENSOR_MODE_PREVIEW;
			spin_unlock(&ov5670mipiraw_drv_lock);
			OV5670_SetDummy_YIWEILI(0, dummyLine);			
			break;			
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
...
UINT32 OV5670MIPIGetDefaultFramerateByScenario_YIWEILI(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
...
UINT32 OV5670FeatureControl_YIWEILI(MSDK_SENSOR_FEATURE_ENUM FeatureId,
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_PERIOD:
				*pFeatureReturnPara16++= OV5670_FeatureControl_PERIOD_PixelNum_YIWEILI;
				*pFeatureReturnPara16= OV5670_FeatureControl_PERIOD_LineNum_YIWEILI;
				*pFeatureParaLen=4;
				break;
        ...
        case SENSOR_FEATURE_SET_ESHUTTER:
            OV5670_SetShutter_YIWEILI(*pFeatureData16);
            break;
        case SENSOR_FEATURE_SET_NIGHTMODE:
            OV5670_NightMode_YIWEILI((BOOL) *pFeatureData16);
            break;
        case SENSOR_FEATURE_SET_GAIN:  
           OV5670_SetGain_YIWEILI((UINT16) *pFeatureData16);
            break;
        ...
        case SENSOR_FEATURE_GET_REGISTER:
            pSensorRegData->RegData = OV5670_read_cmos_sensor_YIWEILI(pSensorRegData->RegAddr);
            break;
        case SENSOR_FEATURE_SET_CCT_REGISTER:
            ...
            {
            	spin_lock(&ov5670mipiraw_drv_lock);
                OV5670SensorCCT_YIWEILI[i].Addr=*pFeatureData32++;
                OV5670SensorCCT_YIWEILI[i].Para=*pFeatureData32++;
				spin_unlock(&ov5670mipiraw_drv_lock);
            }
            break;
        case SENSOR_FEATURE_GET_CCT_REGISTER:
            ...
            *pFeatureData32++=SensorRegNumber;
            for (i=0;i<SensorRegNumber;i++)
            {
                *pFeatureData32++=OV5670SensorCCT_YIWEILI[i].Addr;
                *pFeatureData32++=OV5670SensorCCT_YIWEILI[i].Para;
            }
            break;
        case SENSOR_FEATURE_SET_ENG_REGISTER:
            ...
            {
            	spin_lock(&ov5670mipiraw_drv_lock);
                OV5670SensorReg_YIWEILI[i].Addr=*pFeatureData32++;
                OV5670SensorReg_YIWEILI[i].Para=*pFeatureData32++;
				spin_unlock(&ov5670mipiraw_drv_lock);
            }
            break;
        case SENSOR_FEATURE_GET_ENG_REGISTER:
            ...
            *pFeatureData32++=SensorRegNumber;
            for (i=0;i<SensorRegNumber;i++)
            {
                *pFeatureData32++=OV5670SensorReg_YIWEILI[i].Addr;
                *pFeatureData32++=OV5670SensorReg_YIWEILI[i].Para;
            }
            break;
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            if (*pFeatureParaLen>=sizeof(NVRAM_SENSOR_DATA_STRUCT))
            {
                ...
                memcpy(pSensorDefaultData->SensorEngReg, OV5670SensorReg_YIWEILI, sizeof(SENSOR_REG_STRUCT)*ENGINEER_END);
                memcpy(pSensorDefaultData->SensorCCTReg, OV5670SensorCCT_YIWEILI, sizeof(SENSOR_REG_STRUCT)*FACTORY_END_ADDR);
            }
            else
                return FALSE;
        ...
        case SENSOR_FEATURE_CAMERA_PARA_TO_SENSOR:
            OV5670_camera_para_to_sensor_YIWEILI();
            break;

        case SENSOR_FEATURE_SENSOR_TO_CAMERA_PARA:
            OV5670_sensor_to_camera_para_YIWEILI();
            break;
        case SENSOR_FEATURE_GET_GROUP_COUNT:
            *pFeatureReturnPara32++=OV5670_get_sensor_group_count_YIWEILI();
            *pFeatureParaLen=4;
            break;
        case SENSOR_FEATURE_GET_GROUP_INFO:
            OV5670_get_sensor_group_info_YIWEILI(pSensorGroupInfo->GroupIdx, pSensorGroupInfo->GroupNamePtr, &pSensorGroupInfo->ItemCount);
            *pFeatureParaLen=sizeof(MSDK_SENSOR_GROUP_INFO_STRUCT);
            break;
        case SENSOR_FEATURE_GET_ITEM_INFO:
            OV5670_get_sensor_item_info_YIWEILI(pSensorItemInfo->GroupIdx,pSensorItemInfo->ItemIdx, pSensorItemInfo);
            *pFeatureParaLen=sizeof(MSDK_SENSOR_ITEM_INFO_STRUCT);
            break;

        case SENSOR_FEATURE_SET_ITEM_INFO:
            OV5670_set_sensor_item_info_YIWEILI(pSensorItemInfo->GroupIdx, pSensorItemInfo->ItemIdx, pSensorItemInfo->ItemValue);
            *pFeatureParaLen=sizeof(MSDK_SENSOR_ITEM_INFO_STRUCT);
            break;
        ...
        case SENSOR_FEATURE_SET_VIDEO_MODE:
            OV5670SetVideoMode_YIWEILI(*pFeatureData16);
            break;
        case SENSOR_FEATURE_CHECK_SENSOR_ID:
            OV5670GetSensorID_YIWEILI(pFeatureReturnPara32);
            break;
        case SENSOR_FEATURE_SET_AUTO_FLICKER_MODE:
            OV5670SetAutoFlickerMode_YIWEILI((BOOL)*pFeatureData16, *(pFeatureData16+1));
	        break;
		case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
			OV5670MIPISetMaxFramerateByScenario_YIWEILI((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, *(pFeatureData32+1));
			break;
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
			OV5670MIPIGetDefaultFramerateByScenario_YIWEILI((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, (MUINT32 *)(*(pFeatureData32+1)));
			break;
		case SENSOR_FEATURE_SET_TEST_PATTERN:
			OV5670SetTestPatternMode_YIWEILI((BOOL)*pFeatureData16);
			break;
...
SENSOR_FUNCTION_STRUCT	SensorFuncOV5670_YIWEILI=
{
    OV5670Open_YIWEILI,
    OV5670GetInfo_YIWEILI,
    OV5670GetResolution_YIWEILI,
    OV5670FeatureControl_YIWEILI,
    OV5670Control_YIWEILI,
    OV5670Close_YIWEILI
};

UINT32 OV5670_MIPI_RAW_YIWEILI_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    if (pfFunc!=NULL)
        *pfFunc=&SensorFuncOV5670_YIWEILI;
