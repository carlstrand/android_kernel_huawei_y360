...
int OV5670MIPI_check_awb_gain(){
	...
	R_Gain_H=(UINT8)OV5670_read_cmos_sensor(0x5032);
	R_Gain_L=(UINT8)OV5670_read_cmos_sensor(0x5033);
	G_Gain_H=(UINT8)OV5670_read_cmos_sensor(0x5034);
	G_Gain_L=(UINT8)OV5670_read_cmos_sensor(0x5035);
	B_Gain_H=(UINT8)OV5670_read_cmos_sensor(0x5036);
	B_Gain_L=(UINT8)OV5670_read_cmos_sensor(0x5037);

	OV5670DB("guangzhen test R_Gain_H=0x%02x R_Gain_L=0x%02x G_Gain_H=0x%02x G_Gain_L=0x%02x B_Gain_H=0x%02x B_Gain_L=0x%02x\r\n",
...
int check_otp_info(int index)
{
	...
	mdelay(5);
	flag = OV5670_read_cmos_sensor(0x7010);
	//select group
	if(index==1)
	{
		flag = (flag>>6) & 0x03;
	...
	OV5670_OTP_read_end();
	OV5670DB("guangzhen check_otp_info  flag = %d \n",flag);
	if (flag == 0x00) {
		return 0;
	}
	else if (flag & 0x02) {
...
int check_otp_wb(int index)
{
	...
	//select group
	flag = OV5670_read_cmos_sensor(0x7020);
...
int read_otp_info(int index, struct otp_struct *otp_ptr)
{
	...
	mdelay(5);
	(*otp_ptr).module_integrator_id = OV5670_read_cmos_sensor(address_start);
	(*otp_ptr).lens_id = OV5670_read_cmos_sensor(address_start + 1);
	(*otp_ptr).production_year = OV5670_read_cmos_sensor(address_start + 2);
	(*otp_ptr).production_month = OV5670_read_cmos_sensor(address_start + 3);
	(*otp_ptr).production_day = OV5670_read_cmos_sensor(address_start + 4);
	// clear otp buffer
	for (i=address_start;i<=address_end;i++) {
		OV5670_write_cmos_sensor(i, 0x00);
	}
...
// otp_ptr: pointer of otp_struct
// return: 0,
int read_otp_wb(UINT32 index, struct otp_struct *otp_ptr)
{
	UINT32 i, temp;
	...
	OV5670_write_cmos_sensor(0x3d81, 0x01); // load otp into buffer
	mdelay(5);
	temp = OV5670_read_cmos_sensor(address_start+2);
	otp_ptr->rg_ratio = (OV5670_read_cmos_sensor(address_start)<<2) + ((temp>>6) & 0x03);
	temp = OV5670_read_cmos_sensor(address_start+2);
	otp_ptr->bg_ratio = (OV5670_read_cmos_sensor(address_start + 1)<<2) + ((temp>>4) & 0x03);
	//temp = OV5670_read_cmos_sensor(address_start+21);
	//otp_ptr->r_golden = (OV5670_read_cmos_sensor(address_start + 20)<<2) + ((temp>>6) & 0x03);
	//temp = OV5670_read_cmos_sensor(address_start+23);
	//otp_ptr->b_golden = (OV5670_read_cmos_sensor(address_start + 22)<<2) + ((temp>>6) & 0x03);
	//temp = OV5670_read_cmos_sensor(address_start+25);
	//otp_ptr->gr_golden = (OV5670_read_cmos_sensor(address_start + 24)<<2) + ((temp>>6) & 0x03);
	//temp = OV5670_read_cmos_sensor(address_start+27);
	//otp_ptr->gb_golden = (OV5670_read_cmos_sensor(address_start + 26)<<2) + ((temp>>6) & 0x03);
	// clear otp buffer
	...
	//OV5670DB("AWB otp: gr_golden = %d \n",otp_ptr->gr_golden);
	//OV5670DB("AWB otp: gb_golden = %d \n",otp_ptr->gb_golden);
	OV5670DB("AWB otp: rg_ratio = %d \n",otp_ptr->rg_ratio);
	OV5670DB("AWB otp: bg_ratio = %d \n",otp_ptr->bg_ratio);
...
// return 0;
int update_awb_gain(UINT32 R_gain, UINT32 G_gain, UINT32 B_gain)
{
	...
	OV5670DB("guangzhen update_awb_gain R_Gain_H=0x%x;R_Gain_L= 0x%x;G_Gain_H=0x%x;G_Gain_L=0x%x;B_Gain_H=0x%x;B_Gain_L=0x%x\n",
		R_Gain_H,R_Gain_L,G_Gain_H,G_Gain_L,B_Gain_H,B_Gain_L);
	if (R_gain>=0x400) {
...
// check first OTP with valid data
	...
	printk("guangzhen test OTP index----%d---\r\n",otp_index);
	if (i>3) {
	// no valid wb OTP data
		printk("guangzhen test no wb OTP-------\r\n");
		return 1;
	}
	...
	BG_Ratio_Typical=0x119;//0x120;
	RG_Ratio_Typical=0x127;//0x12c;
	OV5670DB("guangzhen test ration rg bg [0x%x,0x%x]\r\n",rg,bg);
	//OV5670DB("guangzhen test r b gr gb[0x%x,0x%x,0x%x,0x%x]\r\n",r_golden,b_golden,gr_golden,gb_golden);
	//OV5670DB("guangzhen test BG_Ratio_Typical=0x%04x  RG_Ratio_Typical=0x%04x\r\n",BG_Ratio_Typical,RG_Ratio_Typical);
	//calculate G gain
	//0x400 = 1x gain
	...
	OV5670DB("guangzhen test uR_gain=0x%04x,uG_gain=0x%04x,uB_gain=0x%04x\r\n",R_gain,G_gain,B_gain);
	...
	*/
	 *sensorID = 0xFFFFFFFF;
     return ERROR_SENSOR_CONNECT_FAIL;
        ...
        OV5670DB("Read guangzhen Module ID = 0x%x, otp_index=%d\n", current_otp.module_integrator_id,otp_index);
		if(0x04 != current_otp.module_integrator_id){
			 *sensorID = 0xFFFFFFFF;
		     return ERROR_SENSOR_CONNECT_FAIL;
		}
