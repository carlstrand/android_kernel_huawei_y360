...
/*----------------------------------------------------------------------------*/
static int bma_remap_to_gravity(struct i2c_client *client, int data[BMA_AXES_NUM]);
...
static int bma2xx_init_flag = -1; // 0<==>OK -1 <==> fail
...
#endif
	raw[BMA_AXIS_X] = obj->cali_sw[BMA_AXIS_X];
	raw[BMA_AXIS_Y] = obj->cali_sw[BMA_AXIS_Y];
	raw[BMA_AXIS_Z] = obj->cali_sw[BMA_AXIS_Z];
...
/*----------------------------------------------------------------------------*/
static int BMA_WriteCalibration_New(struct i2c_client *client, struct GS_CALI_DATA_STRUCT *data_cali)
{
	struct bma_i2c_data *obj = i2c_get_clientdata(client);
		...
		char *buf, int bufsize)
{
	...
	if (err) {
		...
		/* compensate data */
		if(gs_cali.valid == 1){
			acc[BMA_AXIS_X] -= gs_cali.x;
			acc[BMA_AXIS_Y] -= gs_cali.y;
			acc[BMA_AXIS_Z] -= gs_cali.z;
		}
...
static int bma_remap_to_gravity(struct i2c_client *client, int data[BMA_AXES_NUM])
{
	...
	/*remap coordinate*/
	acc_tmp[obj->cvt.map[BMA_AXIS_X]] = obj->cvt.sign[BMA_AXIS_X]* data[BMA_AXIS_X];
	acc_tmp[obj->cvt.map[BMA_AXIS_Y]] = obj->cvt.sign[BMA_AXIS_Y]* data[BMA_AXIS_Y];
	acc_tmp[obj->cvt.map[BMA_AXIS_Z]] = obj->cvt.sign[BMA_AXIS_Z]* data[BMA_AXIS_Z];

	//Out put the mg
	data[BMA_AXIS_X] = acc_tmp[BMA_AXIS_X] * GRAVITY_EARTH_1000 / obj->sensitivity;
	data[BMA_AXIS_Y] = acc_tmp[BMA_AXIS_Y] * GRAVITY_EARTH_1000 / obj->sensitivity;
	data[BMA_AXIS_Z] = acc_tmp[BMA_AXIS_Z] * GRAVITY_EARTH_1000 / obj->sensitivity;
...
//---------------------------------
	...
	err = bma_remap_to_gravity(client, acc_tmp);
	if(err){
		err = -EFAULT;
		break;
	}
	...
	if (copy_from_user(&gs_cali_temp, data, sizeof(gs_cali_temp))) {
		...
		err = BMA_WriteCalibration_New(client, &gs_cali_temp);
		if(err){
			err = -EFAULT;
			break;
		}
		...
		const struct i2c_device_id *id)
{
	...
	err = bma_create_attr(&(bma2xx_init_info.platform_diver_addr->driver));
	if (err) {
	...
	sobj.self = obj;
	sobj.polling = 1;
	sobj.sensor_operate = bma_gsensor_operate;
...
exit_hwmsen_attach_failed:
	bma_delete_attr(&(bma2xx_init_info.platform_diver_addr->driver));
...
static int bma_i2c_remove(struct i2c_client *client)
{
	...
	err = bma_delete_attr(&(bma2xx_init_info.platform_diver_addr->driver));
	if (err)
...
{
	struct acc_hw *hw = bma_get_cust_acc_hw();
	GSE_FUN();
