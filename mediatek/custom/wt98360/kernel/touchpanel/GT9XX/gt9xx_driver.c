                               ...
                               unsigned long arg)
{
    ...
    if (err)
    {
        GTP_DEBUG("tpd: access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
        return -EFAULT;
    }
    ...
    switch (cmd)
    {
        ...
        default:
            GTP_INFO("tpd: unknown IOCTL: 0x%08x\n", cmd);
            err = -ENOIOCTLCMD;
            break;
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    else
    {
        ...
        if(0 == Module_ID)
        {
            sprintf(tp_string_version,"IC VERSION:%x_%x, sensor_id=0, cfg_version:0x%x", tpd_info.pid, tpd_info.vid, grp_cfg_version);
        }
        else if(1 == Module_ID)
        {
            sprintf(tp_string_version,"IC VERSION:%x_%x, sensor_id=1, cfg_version:0x%x", tpd_info.pid, tpd_info.vid, grp_cfg_version);
        }
        else if(2 == Module_ID)
        {
            sprintf(tp_string_version,"IC VERSION:%x_%x, sensor_id=2, cfg_version:0x%x", tpd_info.pid, tpd_info.vid, grp_cfg_version);
        }
        else if(3 == Module_ID)
        {
            sprintf(tp_string_version,"IC VERSION:%x_%x, sensor_id=3, cfg_version:0x%x", tpd_info.pid, tpd_info.vid, grp_cfg_version);
        }
        else
        {
            sprintf(tp_string_version,"IC VERSION:%x_%x, sensor_id=4, cfg_version:0x%x", tpd_info.pid, tpd_info.vid, grp_cfg_version);
        }
        hardwareinfo_set_prop(HARDWARE_TP, tp_string_version);//set tp hardware info
    }
#endif
