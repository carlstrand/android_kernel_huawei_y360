...
extern int is_tp_sleep;

static ssize_t gtp_open_test_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gtp_open_test_write_proc(struct file *, const char __user *, size_t, loff_t *);

static ssize_t gtp_short_test_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gtp_short_test_write_proc(struct file *, const char __user *, size_t, loff_t *);
...
static const struct file_operations gtp_short_test_proc_ops =
{
    .owner = THIS_MODULE,
    .read = gtp_short_test_read_proc,
    .write = gtp_short_test_write_proc,
};
#endif
...
static ssize_t gtp_open_test_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    //if tp is sleeping, then do not enter test
    if(is_tp_sleep)
    {
        return 0;
    }
    ...
    result=gt9xx_open_test(i2c_client_point);
...
static ssize_t gtp_open_test_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    return 0;
}

static ssize_t gtp_short_test_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;
    s32 result;
    s32 index;
    ...
    result=gt9xx_short_test(i2c_client_point);
...
static ssize_t gtp_short_test_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    return 0;
}
...
s32 gtp_test_sysfs_init(void)
{
    ...
    if(gtp_parent_proc)
    {
        gtp_open_proc= proc_create(OPEN_TEST, 0666, gtp_parent_proc, &gtp_open_test_proc_ops);
        gtp_short_proc= proc_create(SHORT_TEST, 0666, gtp_parent_proc, &gtp_short_test_proc_ops);
    }
#endif
