...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    */
	...
	char *br_ptr;
		...
		temp_cmdline=*cmdline;
		 p = strstr(temp_cmdline, "emmc_name=");
		if(p == NULL)
		{
			printk("cannot find emmc name in cmdline");
		}		
		p += 10;
		printk("emmc p-t=%d,strle=%d",(p - temp_cmdline),strlen(temp_cmdline+1));
		if((p - temp_cmdline) > strlen(temp_cmdline+1))
		{
			printk("error :find emmc name in cmdline");
		}
		q = p;
		while(*q != ' ' && *q != '\0')
			q++;
		memset((void*)emmc_device_name, 0, sizeof(emmc_device_name));
		strncpy((char*)emmc_device_name, (const char*)p, (int)(q-p));
		printk("\r\n wangfuqiang cmdline emmc_device_name %s \r\n",emmc_device_name);
...
struct platform_device sensor_mhall = {
	.name	       = "mhall",
	.id            = -1,
};
...
#if defined(WT_MHALL_SUPPORT)
	retval = platform_device_register(&sensor_mhall);
		printk("sensor_mhall device!");
	if (retval != 0)
		return retval;
#endif
