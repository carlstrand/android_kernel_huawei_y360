...
//    unsigned int status;
	...
	DISPLAY_GAMMA_T gamma_param_drv;
        DISP_PQ_PARAM pq_param_drv;
#endif
...
//            disp_set_needupdate(DISP_MODULE_BLS , 0);
...
#ifdef WT_GAMMA_PQ_WITH_MULTI_LCM
		case DISP_IOCTL_GET_GAMMAPARAM_FROM_LCDDRV:
			 if(DISP_GetGammaParam_LCD(&gamma_param_drv))
			 {
	             if(copy_to_user((void *)arg, &gamma_param_drv, sizeof(DISPLAY_GAMMA_T)))
	             {
		...
		case DISP_IOCTL_GET_PQPARAM_FROM_LCDDRV:
			 if(DISP_GetPQparam_LCD(&pq_param_drv))
			 {
	             if(copy_to_user((void *)arg, &pq_param_drv, sizeof(DISP_PQ_PARAM)))
	             {
