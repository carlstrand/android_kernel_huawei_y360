...
#define NMI_POWER_PIN		(GPIO20 | 0x80000000)
#define NMI_RESET_PIN		(GPIO92 | 0x80000000)
#define NMI_IRQN_PIN		(GPIO17 | 0x80000000)

#define NMI_FMANT_PIN		(GPIO120 | 0x80000000)
#define NMI_DTVANT_PIN		(GPIO121 | 0x80000000)
...
void isdbt_gpio_init(void)
{
	...
	mt_set_gpio_mode(NMI_FMANT_PIN, GPIO_MODE_00);
	mt_set_gpio_dir(NMI_FMANT_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(NMI_FMANT_PIN, 1);

	mt_set_gpio_mode(NMI_DTVANT_PIN, GPIO_MODE_00);
	mt_set_gpio_dir(NMI_DTVANT_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(NMI_DTVANT_PIN, 0);
...
#define NMI_HW_DRVNAME "nmi_hw"

void isdbt_gpio_power_on(void)
{
        if(TRUE != hwPowerOn(MT6323_POWER_LDO_VGP3, VOL_1200,NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
        }      
        
	mt_set_gpio_out(NMI_POWER_PIN, 1);
	mt_set_gpio_out(NMI_FMANT_PIN, 0);
	mt_set_gpio_out(NMI_DTVANT_PIN, 1);
...
void isdbt_gpio_power_off(void)
{
	...
	mt_set_gpio_out(NMI_DTVANT_PIN, 0);
        ...
        if(TRUE != hwPowerDown(MT6323_POWER_LDO_VGP3, NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
        }
...
void isdbt_gpio_interrupt_register(void)
{
	...
	mt_set_gpio_dir(GPIO_DTV_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_mode(GPIO_DTV_EINT_PIN, GPIO_DTV_EINT_PIN_M_EINT);
	mt_set_gpio_pull_enable(GPIO_DTV_EINT_PIN, false);

	mt_eint_set_hw_debounce(CUST_EINT_DTV_NUM, CUST_EINT_DTV_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_DTV_NUM, CUST_EINT_DTV_TYPE, isdbt_irq_handler, 0);
...
void isdbt_gpio_interrupt_unregister(void)
{
	...
	// gpio
	mt_set_gpio_mode(GPIO_DTV_EINT_PIN, GPIO_DTV_EINT_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_DTV_EINT_PIN, GPIO_DIR_IN);
...
{
    NM_KMSG("<isdbt> isdbt_gpio_interrupt_enable, IN [-]\n");
...
{
    NM_KMSG("<isdbt> isdbt_gpio_interrupt_disable, IN [-]\n");
...
// ]]] GPIO
//----------------------------------------------------------------------
static void isdbt_read_gpio()
{
    s32 val = -1;
    mt_set_gpio_mode(GPIO_DTV_EINT_PIN, GPIO_DTV_EINT_PIN_M_GPIO);
    mt_set_gpio_dir(GPIO_DTV_EINT_PIN, GPIO_DIR_IN);

    val = mt_get_gpio_in(GPIO_DTV_EINT_PIN);
    NM_KMSG("~~~~~ <isdbt> val=%d\n", val);
...
static struct platform_device device_isdbt_nmi= {
    .name   = "isdbt",
    .id     = -1,
    .dev    = {
    .platform_data = NULL,
...
static int __init isdbt_init(void)
{
	...
	result = platform_device_register(&device_isdbt_nmi);
	if(result)
		return result;
