...
char hardware_info_lcdname[256] = {0};

#ifdef WT_BACKLIGHT_DISABLE_WITH_NOLCM
int lcm_connected = 1;
#endif
...
BOOL disp_get_lcm_name_boot(char *cmdline)
{
    ...
    strncpy((char*)hardware_info_lcdname, (const char*)p, (int)(q-p));
    
    if(DISP_SelectDeviceBoot(disp_lcm_name))
        ret = TRUE;

#ifdef WT_BACKLIGHT_DISABLE_WITH_NOLCM
	p = strstr(cmdline, "lcm_connected=0");
	if(p != NULL)
	{				
		lcm_connected = 0;		   
	}
	isLCMConnected = lcm_connected;
#endif
...
BOOL DISP_GetGammaParam_LCD(DISPLAY_GAMMA_T *gamma_parameter)
{
	...
	if(lcm_drv->get_gamma_param != NULL)
	{
	    lcm_drv->get_gamma_param((DISPLAY_GAMMA_T_LCD *)gamma_parameter);
...
BOOL DISP_GetPQparam_LCD(DISP_PQ_PARAM *pq_parameter)
{
	...
	if(lcm_drv->get_pqparam != NULL)
	{
	    lcm_drv->get_pqparam((DISP_PQ_PARAM_LCD *)pq_parameter);
